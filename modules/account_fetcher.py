...
def fetch_actual_accounts(actual_client):
    ...
    try:
        ...
        open_actual_accounts = {
            acc.id: {
                key: value for key, value in acc.__dict__.items() 
                if not callable(value) and not key.startswith('_') and is_simple_value(value)
            }
...
def fetch_ynab_accounts():
    ...
    try:
        ...
        accounts_json = requests.get(
            f"{ynab_endpoint}budgets/{ynab_budget_id}/accounts", 
...
def get_akahu_balance(akahu_account_id, akahu_endpoint, akahu_headers):
    ...
    try:
        response = requests.get(
            f"{akahu_endpoint}/accounts/{akahu_account_id}", 
